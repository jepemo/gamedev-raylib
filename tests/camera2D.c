...
Color COLOURS[MAX_COLORS] = { PURPLE, BLUE,  YELLOW, GREEN,   PINK, ORANGE,
                              LIME,   BROWN, BEIGE,  MAGENTA, GOLD };
      ...
      for (int y = 0; y <= Y_BLOCKS; ++y)
        {
          ...
          scene.objects[ind].position
              = (Vector2){ x * SIZE_BLOCK, y * SIZE_BLOCK };
...
RenderScene (Scene *scene)
{
  for (int i = 0; i < MAX_OBJECTS; ++i)
    {
      DrawRectangleV (scene->objects[i].position, scene->objects[i].size,
                      scene->objects[i].color);
    }

  DrawRectangleV (scene->pointer.position, scene->pointer.size,
                  scene->pointer.color);
}

void
UpdateMyCamera (Camera2D *camera, Scene *scene)
{
...
main (int argc, char **argv)
{
  ...
  Camera2D camera = { 0 };
  camera.target
      = (Vector2){ scene.pointer.position.x, scene.pointer.position.y };
